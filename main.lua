        ...
        clothingCombo:onPlayerInOut(function(isPointInside, _, zone)
            if isPointInside then
                ...
                if zoneName == 'surgeon' then
                    exports['vcrp-drawtext']:DrawText('[E] - Plastic Surgery', 'left')
                elseif zoneName == 'clothing' then
                    exports['vcrp-drawtext']:DrawText('[E] - Clothing Shop', 'left')
                elseif zoneName == 'barber' then
                    exports['vcrp-drawtext']:DrawText('[E] - Barber', 'left')
                end
            else
                inZone = false
                exports['vcrp-drawtext']:HideText()
            end
        end)
...
function enableCam()
    ...
    if(not DoesCamExist(cam)) then
        ...
        SetCamUseShallowDofMode(cam, true) -- NP Style -- Depth Of Field Settings
        SetCamNearDof(cam, 1.0)
        SetCamFarDof(cam, 2.0)
        SetCamDofStrength(cam, 0.6)
        SetCamFov(cam, 20.0)
...
RegisterCommand("refreshskin", function()
    ...
    --exports["vcrp-backitems"]:refreshProps()
end)
